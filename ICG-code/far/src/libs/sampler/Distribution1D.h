...
struct __UToT__<T, Vec3<H> >
{
	static inline T convert(const Vec3<H> &u) { return u.Average(); }
};

template<typename T, typename H>
struct __UToT__<T, Vec2<H> >
{
	static inline T convert(const Vec2<H> &u) { return u.Average(); }
};
...
public:
	...
	Distribution1D() { _funcInt = 0; _count = 0; }
    Distribution1D(const U *f, uint32_t n) 
        : _func(n), _cdf(n+1)
    {
        _count = n;
        _cdf[0] = 0.0f;
