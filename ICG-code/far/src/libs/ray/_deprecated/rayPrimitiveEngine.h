#ifndef _RAYPRIMITIVEENGINE_H_
#define _RAYPRIMITIVEENGINE_H_

#include <ray/rayPrimitive.h>

class RayPrimitiveEngine : public RayEngine {
public:
    ...
    friend class RayPrimitiveEngineBuilder;
protected:
...
class RayPrimitiveEngineBuilder {
public:
    shared_ptr<RayPrimitiveEngine> Build(const vector<shared_ptr<Surface> >& surfaces, 
        const vector<shared_ptr<InstanceGroup> >& instances, const Intervalf& time, int timeSamples);

protected:
    virtual void _Group(shared_ptr<RayPrimitiveEngine> engine, shared_ptr<RayEngine> instanceEngine, 
        shared_ptr<Xform> xform, shared_ptr<Material> m, const Intervalf& time);
    virtual shared_ptr<RayEngine> _Subengine(shared_ptr<RayPrimitiveEngine> mainEngine, shared_ptr<Shape> shape, const Intervalf& time, int timeSamples);

    virtual shared_ptr<RayPrimitiveEngine> _CreateEngine(const Intervalf& time) = 0;
    virtual void _InitAcceleration(shared_ptr<RayPrimitiveEngine> engine, const Intervalf& time) = 0;
