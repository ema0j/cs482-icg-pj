...
struct _KdTreeInternalNode : public _KdTreeNode {
    _KdTreeNode* minChild;
    _KdTreeNode* maxChild;
    float split;

    _KdTreeInternalNode(_KdSplitType t, float s, _KdTreeNode* m, _KdTreeNode* M, int depth) {
        type = t;
        split = s;
        minChild = m;
        maxChild = M;
    ...
    virtual ~_KdTreeInternalNode() { if(minChild) delete minChild; if(maxChild) delete maxChild; }
...
struct _KdTreeLeafNode : public _KdTreeNode {
    carray<RayPrimitive*> prims;

    _KdTreeLeafNode(const vector<_KdTreeBoxedPrimitive*>& p, int depth) {
        type = _KD_LEAF;
        prims.resize(p.size());
    ...
    _KdTreeLeafNode(const vector<_KdTreeBoxedAreaPrimitive*>& p, int depth) {
        type = _KD_LEAF;
        prims.resize(p.size());
        for(int i = 0; i < prims.size(); i ++) {
            prims[i] = p[i]->prim;
    ...
    _KdTreeLeafNode(const _PreallocatedArray<_KdTreeBoxedPrimitive*>& p, int depth) {
        type = _KD_LEAF;
        prims.resize(p.size());
        for(int i = 0; i < prims.size(); i ++) {
            prims[i] = p[i]->prim;
    ...
    virtual void CollectStats(StatsManager& stats, int depth) {
        ...
        memory->Increment(sizeof(*this) + prims.size()*sizeof(RayPrimitive*));
    }
};
...
RayKdTreeEngine::RayKdTreeEngine() {
    _root = 0;
...
void RayKdTreeEngine::Print(FILE* f) {
	_root->Print(f, 0);
}

void RayKdTreeEngine::CollectStats(StatsManager& stats) {
    RayPrimitiveEngine::CollectStats(stats);
    _root->CollectStats(stats, 0);
}


bool RayKdTreeEngine::Intersect(const Ray& ray, Intersection* intersection) {
#if (FAR_MULTITHREAD == 0)
    ...
    _KdTreeNode* node = _root;
    RayPrimitive* hitPrimitive = 0;
    while(node && r.tMax >= tMin) {
        if(node->IsLeaf()) {
            // grab primitives
            int nprims = ((_KdTreeLeafNode*)node)->prims.size();
            RayPrimitive** prims = ((_KdTreeLeafNode*)node)->prims.data();
...
#else
				int primMailboxId = (uint32_t)(prims[i]) % _KDTREE_MAX_MAILBOXES;
            ...
            _KdTreeInternalNode* intNode = (_KdTreeInternalNode*)node;
...
bool RayKdTreeEngine::IntersectAny(const Ray& ray) {
#if (FAR_MULTITHREAD == 0)
...
RayKdTreeEngineBuilder::RayKdTreeEngineBuilder() {
    _maxPrims = 5;
    _maxDepth = -1;
}

void RayKdTreeEngineBuilder::_InitAcceleration(shared_ptr<RayPrimitiveEngine> e, const Intervalf& time) {
    shared_ptr<RayKdTreeEngine> kde = dynamic_pointer_cast<RayKdTreeEngine>(e);
    kde->validInterval = time;
    ...
    vector<_KdTreeBoxedPrimitive*> boxedPrims = vector<_KdTreeBoxedPrimitive*>(kde->_primitives.size());
    boxedPrims.resize(kde->_primitives.size());
    for(int i = 0; i < (int)kde->_primitives.size(); i ++) {
        _KdTreeBoxedPrimitive* prim = new _KdTreeBoxedPrimitive();
...
_KdTreeNode* RayKdTreeEngineBuilder::_BuildNode(const Range3f& bbox, const vector<_KdTreeBoxedPrimitive*>& prims, const Intervalf& time, int maxDepth, int depth) {
    // check termination criteria
    if(maxDepth == 0 || (int)prims.size() < _maxPrims) { // check max depth and primitive count
        return _BuildLeafNode(prims, depth);
    } else {
        _KdTreeSplit split = _FindSplit(bbox, prims, time, maxDepth, depth);
        return _BuildInternalNode(bbox, prims, split, time, maxDepth, depth);
    }
}

_KdTreeSplit RayKdTreeEngineBuilder::_FindSplit(const Range3f& bbox, const vector<_KdTreeBoxedPrimitive*>& prims, const Intervalf& time, int maxDepth, int depth) {
    _KdTreeSplit split;
    // find largest axis
...
inline void _SplitBox(const Range3f& bbox, const _KdTreeSplit& split, Range3f& minBox, Range3f& maxBox) {
    minBox = bbox;
    maxBox = bbox;
    switch(split.type) {
        case _KD_X:
...
_KdTreeNode* RayKdTreeEngineBuilder::_BuildInternalNode(const Range3f& bbox, const vector<_KdTreeBoxedPrimitive*>& prims, const _KdTreeSplit& split, const Intervalf& time, int maxDepth, int depth) {
    // split box
    Range3f minBox, maxBox;
    _SplitBox(bbox, split, minBox, maxBox);

    // split primitives into groups
    vector<_KdTreeBoxedPrimitive*> minList, maxList;
    minList.reserve(prims.size()); maxList.reserve(prims.size());
    for(int i = 0; i < (int)prims.size(); i++) {
        _KdTreeBoxedPrimitive* prim = prims[i];
    ...
    if(minList.size() == prims.size() && maxList.size() == prims.size()) {
        node = new _KdTreeLeafNode(prims,depth);
    } else {
        _KdTreeNode *minChild, *maxChild;
        if(minList.size() > 0) {
        ...
        _KdTreeInternalNode* splitnode = new _KdTreeInternalNode(split.type, split.split, minChild, maxChild,depth);
        node = splitnode;
    }
...
_KdTreeNode* RayKdTreeEngineBuilder::_BuildLeafNode(const vector<_KdTreeBoxedPrimitive*>& prims, int depth) {
    return new _KdTreeLeafNode(prims, depth);
}

RayKdTreeFastEngineBuilder::RayKdTreeFastEngineBuilder() {
    _maxPrims = 5;
    _maxDepth = -1;
}

void RayKdTreeFastEngineBuilder::_InitAcceleration(shared_ptr<RayPrimitiveEngine> e, const Intervalf& time) {
    shared_ptr<RayKdTreeEngine> kde = dynamic_pointer_cast<RayKdTreeEngine>(e);
    kde->validInterval = time;

    vector<Range3f> bboxes(kde->_primitives.size()); bboxes.resize(kde->_primitives.size());
    Range3f bbox = _ComputeBoundingBox(kde->_primitives, bboxes, time);
    _PreallocatedArray<_KdTreeBoxedPrimitive*> boxedPrims = _PreallocatedArray<_KdTreeBoxedPrimitive*>(kde->_primitives.size());
    boxedPrims.resize(kde->_primitives.size());
    for(int i = 0; i < (int)kde->_primitives.size(); i ++) {
        _KdTreeBoxedPrimitive* prim = new _KdTreeBoxedPrimitive();
        prim->prim = kde->_primitives[i].get();
    ...
    for(int i = 0; i < _maxDepth; i ++) {
		__minPrims[i] = new _PreallocatedArray<_KdTreeBoxedPrimitive*>(boxedPrims.size());
		__maxPrims[i] = new _PreallocatedArray<_KdTreeBoxedPrimitive*>(boxedPrims.size());
    }
...
_KdTreeNode* RayKdTreeFastEngineBuilder::_BuildNode(const Range3f& bbox, const _PreallocatedArray<_KdTreeBoxedPrimitive*>& prims, const Intervalf& time, int maxDepth, int depth) {
    // check termination criteria
    if(maxDepth == 0 || (int)prims.size() < _maxPrims) { // check max depth and primitive count
        return _BuildLeafNode(prims, depth);
    } else {
...
_KdTreeSplit RayKdTreeFastEngineBuilder::_FindSplit(const Range3f& bbox, const _PreallocatedArray<_KdTreeBoxedPrimitive*>& prims, const Intervalf& time, int maxDepth, int depth) {
    _KdTreeSplit split;
    // find largest axis
    Vec3f size = bbox.GetSize();
    if(size[0] > size[1] && size[0] > size[2]) {
...
_KdTreeNode* RayKdTreeFastEngineBuilder::_BuildInternalNode(const Range3f& bbox, const _PreallocatedArray<_KdTreeBoxedPrimitive*>& prims, const _KdTreeSplit& split, const Intervalf& time, int maxDepth, int depth) {
    // split box
    Range3f minBox, maxBox;
    _SplitBox(bbox, split, minBox, maxBox);

    // split primitives into groups
    // _PreallocatedArray<_KdTreeBoxedPrimitive*> minList(prims.size()), maxList(prims.size());
    _PreallocatedArray<_KdTreeBoxedPrimitive*>& minList = *__minPrims[depth];
    _PreallocatedArray<_KdTreeBoxedPrimitive*>& maxList = *__maxPrims[depth];
...
_KdTreeNode* RayKdTreeFastEngineBuilder::_BuildLeafNode(const _PreallocatedArray<_KdTreeBoxedPrimitive*>& prims, int depth) {
    return new _KdTreeLeafNode(prims, depth);
}
...
struct _KdBBoxEdge {
	...
	_KdBBoxEdge() { }
	_KdBBoxEdge(float edgePos, _KdTreeBoxedAreaPrimitive* p, bool starting) {
		pos = edgePos;
		prim = p;
	...
	float pos;
	_KdTreeBoxedAreaPrimitive* prim;
	enum { BBOXEDGETYPE_MIN = 0, BBOXEDGETYPE_MAX = 1 } type;
};

RayKdTreeSAHEngineBuilder::RayKdTreeSAHEngineBuilder() {
    _maxPrims = 5;
    _maxDepth = -1;
...
void RayKdTreeSAHEngineBuilder::_InitAcceleration(shared_ptr<RayPrimitiveEngine> e, const Intervalf& time) {
    shared_ptr<RayKdTreeEngine> kde = dynamic_pointer_cast<RayKdTreeEngine>(e);
    kde->validInterval = time;
    ...
    vector<_KdTreeBoxedAreaPrimitive*> boxedPrims = vector<_KdTreeBoxedAreaPrimitive*>(kde->_primitives.size());
    boxedPrims.resize(kde->_primitives.size());
    for(int i = 0; i < (int)kde->_primitives.size(); i ++) {
        _KdTreeBoxedAreaPrimitive* prim = new _KdTreeBoxedAreaPrimitive();
        prim->prim = kde->_primitives[i].get();
        prim->bbox = bboxes[i];
        prim->area = areas[i];
        boxedPrims[i] = prim;
...
_KdTreeNode* RayKdTreeSAHEngineBuilder::_BuildNode(const Range3f& bbox, float areaEstimate, const vector<_KdTreeBoxedAreaPrimitive*>& prims, const Intervalf& time, int depth, int badRefines) {
    // check termination criteria
    ...
    for(int axis = 0; axis < 3; axis ++) {
	    ...
	    // Initialize edges
        for (int i = 0; i < (int) prims.size(); ++i) {
            _KdTreeBoxedAreaPrimitive* prim = prims[i];
	...
	if (bestCost > totalIntersectionCost || badRefines == 3) {
        ...
        vector<_KdTreeBoxedAreaPrimitive*> minList, maxList;
        ...
        _KdTreeNode *minChild = _BuildNode(minBox, bestMinArea, minList, time, depth+1, badRefines);
        _KdTreeNode *maxChild = _BuildNode(maxBox, bestMaxArea, maxList, time, depth+1, badRefines);
        node = new _KdTreeInternalNode((_KdSplitType)bestAxis, split, minChild, maxChild, depth);
    }
...
/*
_KdTreeNode* RayKdTreeSAHEngineBuilder::_BuildNode(const Range3f& nodeBounds, vector<_KdTreeBoxedPrimitive*> *prims, const Intervalf& time, int depth, int badRefines) {
    // check termination criteria
	...
	if ((bestCost > 4.f * oldCost && prims.size() < 16) ||
        ...
        _KdTreeNode *minChild = _BuildNode(minBox, &minList, time, depth+1, badRefines);
        _KdTreeNode *maxChild = _BuildNode(maxBox, &maxList, time, depth+1, badRefines);
        node = new _KdTreeInternalNode(split.type, split.split, minChild, maxChild, depth);
    }
